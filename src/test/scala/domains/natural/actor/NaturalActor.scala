...
object NaturalActor extends App with ActorInstance {
  ...
  lazy val actorInstance: ActorType = new Actor[Natural] {
    override val actorDefinition: ActorDefinition = parser.parse(actorSourceContent).flatMap(_.as[ActorDefinition]).getOrElse(ActorDefinition.Null)
