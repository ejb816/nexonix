...
class TupleFactReteTest extends AnyFunSuite {
  ...
  test("TupleFactReteTest") {
    val resourcePath = "org/nexonix/rete/rules/TupleFactRule.json"
    ...
    val ruleSource: String = Generator.generate(rule, Seq("org", "nexonix", "rete"))
    val contentSink: ContentSink = ContentSink(Generator.test.sinkRoot, "org/nexonix/rete/rules/TupleFactRule.scala")
