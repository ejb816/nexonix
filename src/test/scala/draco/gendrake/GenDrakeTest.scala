 ...
 *  a `TypeDefinition` in working memory becomes an `Emission`, the target-neutral product — and the
 *  super-domain `generator.Generator` owns `Emission` and `EmissionReceived`, the rule that gives an
 *  inserted `Emission` a working-memory node and hands it to whoever set "received" on
 ...
 *  draco` is `DrakeGenTest`-pinned to `Drake.emit` of its `.json`, so an `Emission` that
 ...
 *     owns: `Emit` (its own) and `EmissionReceived` (its super's). Counted in the
 *     projected source, not the definitions, for the reason the forest's gate 3 gives.
 *  2. RUNS — insert every definition, fire, and the received emissions ARE the corpus. */
class GenDrakeTest extends AnyFunSuite with PersistentTestLog {
  ...
  test("GenDrake runs: every definition crosses to an Emission that is its own .drake") {
    ...
    assert(received.size == pairs.size, s"expected one Emission per definition, got ${received.size} of ${pairs.size}")
    assert(missing.isEmpty && extra.isEmpty, s"${missing.size} emissions missing, ${extra.size} not in the corpus — see the log")
  }
}
