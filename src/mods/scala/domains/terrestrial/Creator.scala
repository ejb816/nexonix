...
object Creator extends App with DracoType {
  override lazy val typeDefinition: TypeDefinition = TypeLoader.loadType(TypeName ("Creator", _namePackage = Seq ("domains", "terrestrial")))
  ...
  def actorType(_consumer: => draco.format.json.JSON => Unit): ActorType = new Actor[draco.format.json.JSON] {
