...
object CLI extends DracoType {
  ...
  def generate(path: String): Unit = println(genscala.GenScala.generator(load(path)))
  def drake(path: String): Unit = println(draco.gendrake.GenDrake.generator(load(path)))
