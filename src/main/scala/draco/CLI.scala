...
object CLI extends DracoType {
  ...
  def load(_path: => String): TypeDefinition = {
  ...
  def inspect(_path: => String): Unit = {
    lazy val path: String = _path
    println(TypeDefinition.encoder(load(path)).spaces2)
  }
  def generate(_path: => String): Unit = {
    lazy val path: String = _path
    println(DracoGenerator.generate(load(path)))
  }
  def drake(_path: => String): Unit = {
    lazy val path: String = _path
