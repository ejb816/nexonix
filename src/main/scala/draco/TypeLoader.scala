...
object TypeLoader extends App with DracoType {
  ...
  def loadFromResource(_resourcePath: => String): Option[TypeDefinition] = {
    lazy val resourcePath: String = _resourcePath
    draco.generator.carrier.DefinitionPath.default.source(resourcePath).flatMap(readDefinition)
  }
  def tryLoad(_typeName: => TypeName): Option[TypeDefinition] = {
