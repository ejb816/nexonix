...
object TypeLoader extends App with DracoType {
  ...
  def loadFromResource(resourcePath: String): Option[TypeDefinition] = draco.generator.carrier.DefinitionPath.default.source(resourcePath).flatMap(readDefinition)
