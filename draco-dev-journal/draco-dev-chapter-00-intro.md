...
**Status at Chapter 74 (session in progress):** #51's derivation edge closed with no shape change (`324556c`, 525/525); nominal-over-structural and flat TypeParameter decided; the 12-actor migration landed (`05b14bd`, 526/526); the Evrete co-declaration probe produced #63's verdict (co-declared fact types related by inheritance silently drop the insert); the Forest scenario — containment-not-inheritance, leaf-conversion transforms, mixin-tag payloads — parses 22/22 clean; the final commit/push exchange pends.
...
### [Chapter 74 — #51 Closed, the Subtype Visibility Verdict, the Forest Scenario (in progress)](draco-dev-chapter-74.md)

The derivation edge closed with no shape change, then the session widened: nominal-over-structural and flat TypeParameter decided, the Haskell subtyping dialogue split `<:` into obligation/constraint/substitutability, and Dev's aspect-substitutability correction drove the 12-actor migration. The Evrete probe delivered #63's verdict — co-declared fact types related by inheritance silently drop the insert — written up in the issue and here, per Dev's directive. The nine-exchange Forest scenario design (containment, not inheritance; leaf-conversion transforms; mixin-tag payloads) parsed clean, and the session closed by coordinating live with this very journal update. Session in progress.
