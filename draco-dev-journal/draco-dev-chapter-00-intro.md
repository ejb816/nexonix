...
**Status at Chapter 75 (session in progress):** transform-ness became aspect presence — `source`/`target` moved to `domainAspect` with drake keywords (`83d87ac`); the forest scenario became a corpus and ran (`87a2bb9`, 531 tests / 41 suites), after Dev's catch that drake wasn't being parsed into generated JSON flushed two real defects; DRACO.md gained its report-only baselines table with a same-commit maintenance rule (`9616748`); the full-result-paste protocol was agreed. #51's two tracks and #63's body correction remain open; the session is still running.
